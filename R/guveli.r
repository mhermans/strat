...
r.isco88.guveli <- function(x, detail) {

  x$egp <- recode(x, informat="isco88", outformat="egp")  
  x$guveli <- x$egp + 2 # shift categories from 3, lower 
  x$guveli[x$guveli %in% 1:4] <- NA

  # Ia. High-grade Technocrats 
  x$guveli[x$egp == 1 & x$oug %in% c(1000, 1100, 1110, 1120, 1200, 
  ...
  x$guveli[x$egp == 1 & x$oug %in% c(2141, 2213, 2220, 2221, 2222, 
    2223, 2224, 2229, 2310, 2350, 2351, 2352, 2400, 2421, 2422, 
    2429, 2440, 2441, 2442, 2445)] <- 2

  # IIa. Low-grade Technocrats 
  x$guveli[x$egp == 2 & x$oug %in% c(1130, 1140, 1141, 1142, 1143, 
  ...
  x$guveli[x$egp == 2 & x$oug %in% c(2230, 2300, 2320, 2321, 
