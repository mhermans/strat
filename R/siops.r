...
r.isco88.siops <- function(x, detail=0) {
  ...
  x$siops[x$oug %in% c(5131, 6151, 6152, 9100, 9140, 9200, 9210, 9211, 
    9213)] <- 23
  ...
  x$siops[x$oug %in% c(5210, 6150, 6153, 7129, 7134, 7320, 7410, 8124, 8140, 
    8142, 8143, 8223, 8253, 8260, 8266, 8286, 8334)] <- 28
  x$siops[x$oug %in% c(3240, 3241, 5113, 5139, 5140, 5149, 7142, 7420, 7421, 
    7431, 8141, 8261, 8262, 8312, 8340)] <- 29
  x$siops[x$oug %in% c(4131, 5169, 6134, 8170, 8171, 8172, 8212, 8230, 8231, 
    8232, 8281, 8284)] <- 30
  x$siops[x$oug %in% c(5122, 5200, 7131, 7132, 7133, 7140, 7141, 7323, 7324, 7330, 7331, 
    7332, 7437, 8110, 8113, 8130, 8131, 8139, 8240, 8271, 8285, 8321, 8322, 8331)] <- 31
  x$siops[x$oug %in% c(3473, 4130, 5000, 5100, 5110, 5112, 5141, 5220, 7215, 7345, 7432, 
    8112, 8252, 8320, 8323, 8330, 8332)] <- 32
  x$siops[x$oug %in% c(3474, 4142, 7312, 7400, 7412, 8270, 8273, 8274, 8280, 8290, 8300, 
    8324, 8333, 8400)] <- 33
  x$siops[x$oug %in% c(4211, 4213, 5143, 7100, 7110, 7111, 7113, 7120, 7122, 7123, 7136, 
  ...
  x$siops[x$oug %in% c(4000, 4100, 4133, 4140, 4144, 4190, 4210, 5121, 5150, 5151, 5152, 
    5160, 6000, 7124, 7130, 7220, 7322, 7530, 8155, 8210)] <- 37
  x$siops[x$oug %in% c(1315, 4220, 4221, 4222, 4223, 6100, 6130, 6200, 6210, 7000, 7210, 
    7211, 7223, 7240, 7241, 8123, 8160, 8211)] <- 38
  x$siops[x$oug %in% c(3112, 4200, 5163, 5164, 7212, 7300, 8279)] <- 39
  x$siops[x$oug %in% c(2454, 5162, 6110, 6111, 6112, 6113, 6114, 6120, 6121, 6122, 6123, 
    6124, 6129, 6131, 6133, 7200, 7222, 7422, 7433, 7435, 8120)] <- 40
  x$siops[x$oug %in% c(4143, 6132, 7342, 7343, 8250, 8251)] <- 41
  x$siops[x$oug %in% c(3420, 3429, 4111, 4112, 4212, 5132, 7233, 7243, 7340, 7341, 7344, 
    8150, 8161)] <- 42
  x$siops[x$oug %in% c(3414, 7230, 7231, 7313, 8151, 8152, 8153, 8154, 8159, 8220, 8221, 
   8222, 8224, 8229, 8311)] <- 43
  x$siops[x$oug %in% c(3139, 3225, 3228, 3230, 3231, 3232, 3412, 3441, 3452, 4120, 4132, 
    7137, 7214)] <- 44
  x$siops[x$oug %in% c(2453, 3229, 3450, 3470, 4110, 4113, 4114, 4121, 7310, 8121, 8276)] <- 45
  x$siops[x$oug %in% c(1314, 3111, 3113, 3114, 3115, 3116, 3119, 3130, 3131, 3145, 3415, 
  ...
  x$siops[x$oug %in% c(1300, 1310, 3144, 3300, 3310, 3320, 3330, 3340, 3411, 3422, 3472, 
    3480, 5111, 7232)] <- 50
  x$siops[x$oug %in% c(1000, 2130, 2131, 2132, 2139, 3200, 3220, 3226, 3434)] <- 51
  x$siops[x$oug %in% c(1312, 1313, 1316, 1317, 1318, 1319, 2446, 3210, 3211, 3223, 3440, 
    3442, 7346)] <- 52
  ...
  x$siops[x$oug %in% c(1200, 1221, 1222, 1223, 1224, 1225, 1226, 1227, 1228, 1229, 1230, 
